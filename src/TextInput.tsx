...
export const TextInput = () => {
    ...
    const [ShowText, setShowText] = useState("`The following is a conversation with an AI assistant. The assistant is helpful, creative, clever, and very friendly.\n\nHuman: Hello, who are you?\nAI: I am an AI created by OpenAI. How can I help you today?\n`");
    ...
    const con = (con_text:string) => {
        ...
        const returnData:Promise<string> = Chat(ShowText + "Human: " + con_text + "\n")
        returnData
        .then((result) => {
            setShowText(result)
            setText("")
    ...
    return (
        <div className='startmanuebar'>
            ...
            {/* <form onSubmit={(event) => {setText2(event.target.value);console.log(text2)}}> */}
                <label>
                    <input type="text" value={text} onKeyPress={(event)=>{if (event.key == 'Enter'){event.preventDefault();con(text);}}} onChange={(event) => {setText(event.target.value)}}/>
                </label>
